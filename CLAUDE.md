...
- **Eight pages**, plus a 404: `/`, `/kwiaty-balkonowe/`, `/rabatowe/`, `/bratki/`,
  `/chryzantemy/`, `/inspiracje/`, `/o-nas/`, `/kontakt/`. All three of the old WordPress
  offer addresses are kept unchanged; `/bratki/` is the one offer address with no
  predecessor.
...
| `src/data/navigation.ts` | The menu (8 items), derived from `offer.ts`. Also `offerPages` and `footerOfferLinks`. |
...
| `src/content.config.ts` | The zod schemas for `plants` and `pages`. |
| `src/layouts/BaseLayout.astro` | The one layout: head, skip link, header, `<main>`, footer, JSON-LD. |
| `src/components/` | 18 components. `SeasonCards`, `OfferOverview` (home tiles), `OfferSection` (a whole category page), `PlantEntry`, `PhotoSlot` (a pending photograph), `PhotoStrip`, `Compositions` (the slideshow), `Intro`, `Header`, `Nav`, `Footer`, `Contact`, `Directions`, `MapEmbed`, `ConsentBanner`, `History`, `FacebookNews`, `Seo`. |
| `src/scripts/` | The only JavaScript sent to the browser: `consent.ts` (map consent), `compositions.ts` (the slideshow), `lightbox.ts` (the overlay preview). |
...
- **The design is deliberately austere.** No border radius, no shadows, no counters, no
  ...
  gradient) if the photograph, the scrim padding or the heading length changes.
