...
| `src/scripts/`                 | The only JavaScript sent to the browser: `consent.ts` (map consent), `compositions.ts` (the slideshow), `lightbox.ts` (the overlay preview, which also steps through the set), `nav.ts` (closing the menu panel - an enhancement, never a dependency).                                                                                                                                                                             |
...
- **A page gets one photo strip, and the overline lives in `OfferSection`.** All three category
  ...
  - **The strip is lightboxed, and the entries deliberately are not.** Every frame in a
    ...
    **The overlay steps through the set and wraps** - "Poprzednie" / "Następne", the arrow keys,
    and a position readout - which is safe only because **no page mixes two lightbox groups**: a
    category page has its strip, `/inspiracje/` and the home page have the slideshow. Put both on
    one page and "next" walks out of one into the other; the fix then is to scope the set to the
    opener's container, and `lightbox.ts` says so at the top. The controls sit in a bar above the
