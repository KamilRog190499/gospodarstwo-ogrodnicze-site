...
- **A page gets one photo strip, and the overline lives in `OfferSection`.** All three category
  ...
  - **The strip is lightboxed, and the entries deliberately are not.** Every frame in a
    ...
    **The overlay steps through the set and wraps** - "Poprzednie" / "Następne", the arrow keys,
    and a position readout. **The set is the openers that share a `[data-lightbox-group]`
    container**, resolved at click time: a category page has its strip, `/inspiracje/` has the
    slideshow, and the home page has two groups - the photo show and the slideshow - which is
    what ended the older rule that no page mixed two. A group that marks nothing falls back to
    the whole document, so an unmarked page behaves exactly as before. Anything that hides a
    frame has to leave it in the DOM for this to keep working, and has to be `inert` rather than
    merely invisible, or focus returns to a frame nobody can see - `GalleryShow` does both. The controls sit in a bar above the
    photograph and one below it, never on it - this site puts no text on a photograph, and a
    floating arrow would be the first thing to.
